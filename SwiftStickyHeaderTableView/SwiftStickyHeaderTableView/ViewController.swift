...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    register(nibCells: ["StandardCell", "VehicleCell", "PersonCell"])
    loadData(file: "data1")
    edgeInset = UITableViewCell.getDefaultSeparatorInsets()
    tableView.separatorStyle = .none
    stickyTableView.separatorStyle = .none
  ...
  func register(nibCells: [String]) {
    for cell in nibCells {
      tableView.register(UITableViewCell.self, forCellReuseIdentifier: cell)
      stickyTableView.register(UITableViewCell.self, forCellReuseIdentifier: cell)
    }
  }
  ...
  private func loadData(file: String) {
    ...
    if let url = Bundle.main.url(forResource: file, withExtension: "json"),
      ...
      var tmp = json.map { Section(json: $0) }
      sections = [
        tmp.reduce(Section(), { partialResult, section in
          partialResult.rows += section.rows
...
extension ViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if item.cellIdentifier == "VehicleCell", let data = item as? VehicleRow {
    ...
    else if item.cellIdentifier == "PersonCell", let data = item as? PersonRow {
      cell.contentConfiguration = UIHostingConfiguration {
        Text(data.title)
      }
    }
    else if item.cellIdentifier == "StandardCell", let data = item as? StandardRow {
      cell.contentConfiguration = UIHostingConfiguration {
